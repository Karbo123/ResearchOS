...
- 设置面板一级标签固定为 `通用 -> 模型 -> 系统`：`通用` 只放外观（界面语言与主题，全局），`模型` 放全部模型设置，`系统` 放全局代理。除外观外所有设置都是**项目级**的（代码三档、文档文本、图片识别、图片生成、Embedding、语音识别），模型/语音覆盖按项目保存在 `projects/<project-id>/.researchos/model-settings.json`（0600、原子写入、删除项目时清理，`.researchos/` 已加入项目 `.gitignore`），没有覆盖时回退 `.env` 默认；旧 `runtime/project-settings.json` 只作兼容回退，新保存或删除时自动清理对应 key。模型页在未打开项目时提示先打开项目。全局代理仍保存在 `runtime/model-settings.json`。
- 模型设置还包含 `vision` 图片识别模型（默认 `mimo-v2.5` + `http://10.31.107.77:3000/v1`，key 留空回退 medium）与 `image_generation` 图片生成模型（默认 `gpt-image-2-official` + `https://api.apimart.ai/v1`，可配置 1k/2k/4k 与 low/medium/high，默认 1k/low/n=1）。二者与代码/文档模型一样只把 key 写入项目级 `projects/<project-id>/.researchos/model-settings.json`，读取接口只返回 `key_configured`。聊天或 Idea 讨论包含图片附件时，Idea 澄清优先使用 vision 模型，仍失败关闭；图片生成只通过 `/images/generations` 兼容接口调用。每个已配置模型都提供“测试连接”按钮，只发送最小请求验证连通性（图片生成提交一个最省钱的真实任务，不下载不展示）。代码三档、文档文本与图片识别模型名称在填写 URL 与 API key 后通过 `POST /api/projects/:projectId/settings/model-catalog` 自动获取模型列表（请求 `GET <base>/models`，经全局代理，失败关闭），界面用下拉选择模型，推理强度下拉消费目录返回的档位；未获取到列表时保留当前值并显示结构化错误，不静默替换。
...
- 运行时只读取项目 `.env`、`<RESEARCH_RUNTIME_DIR>/model-settings.json`（未设置该变量时默认 `runtime/model-settings.json`；API 与 Mastra 共用同一路径，保存全局代理）、`projects/<project-id>/.researchos/model-settings.json`（保存项目级模型/语音覆盖；旧 `runtime/project-settings.json` 仅作兼容回退）、`runtime/project-embedding-settings.json` 和 `runtime/embedding-pools.json`，不得读取 Codex 配置目录或 `auth.json`。
