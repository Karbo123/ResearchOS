...
- React 工作区导航固定为科研流程：首页是“全部项目”控制台，不承载 Idea 对话；进入项目后左侧项目列表默认收起，通过边缘箭头滑入/滑出，右侧始终保留唯一一个跨页面持续存在的项目级多轮对话。第一栏只能按 `项目概述 -> 相关工作调研 -> 实验实现 -> 学术论文撰写` 排列；项目概述二级固定为 `项目总览 -> Idea 讨论 -> 待审批与决策 -> 定期汇报与反馈`；相关工作调研二级固定为 `文献列表 -> 研究可视化 -> 种子文献扩展`；实验实现只保留 `本方法实现`、`相关工作实现` 两个二级标签，不使用第三级标签栏，两个页面共用“左侧实验列表 + 右侧实验详情”布局，实验相关部分（复现、比较、计划、队列、指标、产物可视化、谱系）只能位于 `实验实现`；`学术论文撰写` 二级固定为 `引言 -> 相关工作 -> 方法介绍 -> 实验 -> 结论`，引用、BibTeX、图表与实验数据选择、LaTeX 编译、PDF 呈现是章节工作区内的能力，不得作为顶层标签，也不承载实验管理/可视化。前端不得使用“导师/老师”等角色文案。项目工作区正常使用无井号的 `/project/<semantic-slug>/<area>/<tab>` History API 地址，URL 中的 `idea` 映射到内部 Idea Tab；旧标签 hash 地址仍只做兼容映射，旧 UUID 项目地址不再兼容、不解析、不重定向。导航重构按 `TODO.md` 的 `P0-WORKSPACE-108A` 推进，完成前不得把旧标签结构重新写回 README 或 AGENTS。
- 项目的唯一标识是语义 slug（如 `native-acceptance-a8b9`），它同时是数据库 `projects.id`、全部项目级 `project_id` 外键、项目目录、v2 workflow definition/缓存、Supermemory container tag 和项目设置键的唯一形式。禁止用 UUID 作为任何项目的内部标识，也不得生成 UUID 作为新项目 ID；旧语义地址（不含 UUID）只在 `project_slug_aliases` 中保留为兼容别名。旧 UUID 不得作为项目引用、别名键、迁移来源或兼容路由，遇到 UUID 项目标识直接 404。非项目实体（paper、proposal、experiment、evidence 等）继续使用 UUID。
...
- 相关工作调研允许阅读并复用用户自有项目 `/mnt/d/auto-related-work`（即 Windows `D:\auto-related-work`）中的算法、字段 schema、缓存策略、测试思路和数据处理设计等部分；“复用”指把可用逻辑翻译成 TypeScript 后纳入 Research OS，不得把该项目的 Python 文件、Python 运行时、旧缓存或 Python 业务模块直接作为 Research OS 依赖。
- 相关工作引擎的 TypeScript 目标能力包括：Crossref/OpenAlex/Semantic Scholar/DBLP/arXiv/Unpaywall source adapters、完整 BibTeX 解析（作者、venue、年份、DOI、摘要）、用户种子文献、depth/width/max_total 引用递归、标题归一化与跨来源去重、多轮字段补全与 Python 完整度阈值、作者机构增量合并、arXiv API 完整作者/摘要与 arXiv HTML5 作者机构/通讯作者解析、引用图、研究现状矩阵、代码仓库候选、取消/进度事件、缓存和结构化失败等等。字段 provenance 必须区分 `provider`、`user_input`、`controlled_artifact`，provider 补全必须通过 DOI 或标题匹配；多 provider 是显式可审计的来源策略，不是模型或服务失败后的静默 fallback。代码复现完成后，TypeScript 只能根据固定 commit、相同数据/配置/seed 和真实 Artifact 计算效果比较；Mastra 可以提出有证据链接的待核验创新/研究空白候选，但用户确认前不得把候选写成创新结论或论文事实。
...
- Memory v2 的第一阶段已经提供 `projects/<project-id>/research/**/*.md` 知识文档注册基础：front matter 必须符合 `researchos/knowledge-document@1`，路径按文档 kind allowlist，文档 ID 使用可读的 namespace/value 形式，项目不匹配、重复 ID、路径穿越和符号链接都失败关闭。`knowledge_documents`、`knowledge_document_revisions`、`knowledge_index_generations`、`knowledge_index_entries` 只保存 hash、状态、版本、定位和索引账本，不复制 Markdown 正文；AST 分块保留标题、原始换行和行号。Supermemory 的代际替换、Context Planner 和科研文档接线仍以 `TODO-memory.md` 为准，未完成前不得宣称 Memory v2 全部实现。
...
- 设置面板一级标签固定为 `通用 -> 模型 -> 系统`：`通用` 只放外观（界面语言与主题，全局），`模型` 放全部模型设置，`系统` 放代理地址信息并提供代理连通性测试。除外观外所有设置都是**项目级**的（代码三档、文档文本、图片识别、图片生成、Embedding、语音识别），每个模型都带独立 `use_proxy` 开关（默认：代码/文档/Embedding 直连，图片识别/图片生成/语音识别走代理），模型/语音覆盖按项目保存在 `projects/<project-id>/.researchos/model-settings.json`（0600、原子写入、删除项目时清理，`.researchos/` 已加入项目 `.gitignore`），没有覆盖时回退 `.env` 默认；旧 `runtime/project-settings.json` 只作兼容回退，新保存或删除时自动清理对应 key。模型页在未打开项目时提示先打开项目。代理地址保存在 `runtime/model-settings.json`，模型测试、模型目录、Mastra Agent、语音转写与远程 Embedding 出口都按各自开关路由；回环/RFC1918 始终直连。
- 模型设置还包含 `vision` 图片识别模型（默认 `mimo-v2.5` + `http://127.0.0.1:3000/v1`，key 留空回退 medium）与 `image_generation` 图片生成模型（默认 `gpt-image-2-official` + `https://api.apimart.ai/v1`，可配置 1k/2k/4k 与 low/medium/high，默认 1k/low/n=1）。二者与代码/文档模型一样只把 key 写入项目级 `projects/<project-id>/.researchos/model-settings.json`，读取接口只返回 `key_configured`。聊天或 Idea 讨论包含图片附件时，Idea 澄清优先使用 vision 模型，仍失败关闭；图片生成只通过 `/images/generations` 兼容接口调用。每个已配置模型都提供“测试连接”按钮，只发送最小请求验证连通性（图片生成提交一个最省钱的真实任务，不下载不展示）。代码三档、文档文本与图片识别模型名称在填写 URL 与 API key 后通过 `POST /api/projects/:projectId/settings/model-catalog` 自动获取模型列表（请求 `GET <base>/models`，按该模型的代理开关路由，失败关闭），界面用下拉选择模型，推理强度下拉消费目录返回的档位；未获取到列表时保留当前值并显示结构化错误，不静默替换。
- 设置面板通用页的语言与主题是草稿状态：点击后不会立即生效，必须点击“保存配置”才写入 localStorage 并应用；关闭或切换页面未保存时保持原值。通用页只在设置面板内出现，不出现在网页右上角。
- 项目工作流采用项目级 v2 声明式运行时：每个项目一份 `projects/<project-id>/workflow.ts`（属于项目内 Git），新项目由默认模板初始化，已有项目用幂等脚本补齐；原生 API loader 默认每 2000ms 轮询哈希热加载到 `runtime/workflow-v2-cache/<project-id>/`（`/mnt/d` 无 inotify，所以热加载只覆盖 `workflow.ts` 本身，loader/运行时源码变化仍需重启 API）。默认模板是科研语义有向图，包含项目上下文、对话、文献、方法与实验、论文五章、汇报、治理和工作流编辑等语义分组；它不是串行流水线，也不是动作 if-else 路由器，分组内部允许真实顺序依赖和并行（例如论文五章并行）。任何 workflow 修改都必须先生成可审阅 diff Proposal，经临时校验、用户审批和项目 Git 提交后才生效；非法文件保留上一有效版本并返回结构化错误。项目页图面板以 `WorkflowGraphSnapshot` 为数据源，通过 `/api/projects/:projectId/workflow/*` 与 SSE 订阅；Mastra Studio 仅作 Agent 与本地调试的开发辅助。运行时状态持久化在 PGlite（`project_workflow_runtime`、`workflow_definitions`、`workflow_events`、`workflow_node_runs` 和任务队列），不再使用 `runtime/workflow-runs.json`；删除项目时同步清理上述表、definition 缓存、设置与项目目录。
- API 的 SSE 统一使用项目内原生 `ReadableStream` 生命周期实现：producer 必须保持到正常结束或客户端取消，取消后通过 `AbortSignal` 终止休眠/轮询；禁止重新使用 Hono `streamSSE` 依赖补丁，也禁止前端用静默 REST polling 掩盖 SSE 断线。业务 PGlite 固定为 `0.3.10`（`0.3.7` 有上游已修复的 `MessageContext` 泄漏，`0.5.x` 不能直接打开当前 `0.3.x` 数据目录）；任何 PGlite 升级必须先复制真实数据目录并验证打开、查询、事务和关闭，不能直接试验或重建运行库。任务 worker 使用单调度器、并发槽位、空闲指数退避和仅运行时心跳；WASM `RuntimeError`、`Aborted()`、`memory access out of bounds` 或已关闭 PGlite 属于进程级致命错误，禁止继续向同一死数据库写“恢复”状态或用全局 `unhandledRejection` 维持假存活。
- 项目范围内的项目对话、论文翻译/修订和实验规划公开 API 统一追加 v2 项目事件并等待有限节点运行；Mastra 继续以 Agent/Skill/Tool 执行模型与文本工作，不再承载项目级总 workflow 运行时。项目创建前的 Idea 澄清仍由 Idea Agent 直接处理，不属于项目 workflow。
- 运行时只读取项目 `.env`、`<RESEARCH_RUNTIME_DIR>/model-settings.json`（未设置该变量时默认 `runtime/model-settings.json`；API 与 Mastra 共用同一路径，保存代理地址）、`projects/<project-id>/.researchos/model-settings.json`（保存项目级模型/语音覆盖及 `use_proxy`；旧 `runtime/project-settings.json` 仅作兼容回退）、`runtime/project-embedding-settings.json` 和 `runtime/embedding-pools.json`，不得读取 Codex 配置目录或 `auth.json`。
...
- 上传只做大小、扩展名和会话数量/总量限制；上传材料默认视为不可信上下文，不执行 Windows Defender 强制扫描，也不执行上传文件。
...
主链、模型、Mastra、实验、数据库或产物谱系变化还必须运行 `npx tsx scripts/acceptance-test.ts`；工作流运行时或审批链路变化还必须运行 `npm run workflow:v2:check`。真实模型无效时记录结构化失败，不得伪造通过。
