...
- React 工作区导航固定为科研流程：第一栏只能按 `项目概述 -> 相关工作调研 -> 实验实现 -> 学术论文撰写` 排列；第二栏显示当前一级入口的工作区；`实验实现` 使用第三级具体页面，二级固定为 `本方法实现`、`相关工作实现`，第三级位于二级栏下方而不是正文内部。实验相关部分（复现、比较、实验计划、队列、指标、产物可视化、谱系）只能位于 `实验实现`；日报/周报与导师反馈位于 `项目概述`；`学术论文撰写` 只承载写作相关页面（论文项目、大纲与章节、引用与 BibTeX、图表与实验数据选择、LaTeX 编译、PDF 呈现），不得承载实验管理/可视化。项目工作区正常使用无井号的 `/project/<semantic-slug>/<area>/<tab>` History API 地址，URL 中的 `idea` 映射到内部 Idea Tab；旧 hash 和旧 UUID 地址只能兼容映射到对应内部页面，不能恢复被淘汰的长导航结构。导航重构按 `TODO.md` 的 `P0-NAV-078` 推进，完成前不得把旧标签结构重新写回 README 或 AGENTS。
...
- 相关工作调研允许复用用户自有项目 `D:\auto-related-work` 的算法、字段 schema、缓存策略、测试思路和数据处理设计，但必须全部重写为 TypeScript；不得把该项目的 Python 文件、Python 运行时或 Python 业务模块直接作为 Research OS 依赖。旧项目中的硬编码代理、外部密钥和 Google Scholar 隧道不能直接带入，必须改成当前项目的显式配置、合法来源适配器和结构化失败。
- 相关工作引擎的 TypeScript 目标能力包括：Crossref/OpenAlex/Semantic Scholar/DBLP/arXiv source adapters、用户种子文献、depth/width/max_total 引用递归、标题归一化与跨来源去重、字段补全与完整度评分、引用图、研究现状矩阵、代码仓库候选、取消/进度事件、缓存和结构化失败。字段 provenance 必须区分 `provider`、`user_input`、`controlled_artifact`，provider 补全必须通过 DOI 或标题匹配；多 provider 是显式可审计的来源策略，不是模型或服务失败后的静默 fallback。代码复现完成后，TypeScript 只能根据固定 commit、相同数据/配置/seed 和真实 Artifact 计算效果比较；Mastra 可以提出有证据链接的待核验创新/研究空白候选，但用户确认前不得把候选写成创新结论或论文事实。
