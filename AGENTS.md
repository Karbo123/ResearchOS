...
- 运行时只读取项目 `.env`、`<RESEARCH_RUNTIME_DIR>/model-settings.json`（未设置该变量时默认 `runtime/model-settings.json`；API 与 Mastra 共用同一路径）、`runtime/project-embedding-settings.json` 和 `runtime/embedding-pools.json`，不得读取 Codex 配置目录或 `auth.json`。
