...
- 仓库只有**一份**：`D:\ResearchOS` 即 WSL2 内的 `/mnt/d/ResearchOS`（同一份文件，不需要同步）。API `8080`、Mastra `4111`、Supermemory `6767` 与项目配置池实例（6770–6869）都从 `/mnt/d/ResearchOS` 启动。由于 `/mnt/d`（drvfs）无 inotify 支持，Windows 侧编辑后 `tsx watch` 不会自动重启，必须手动重启对应服务（见 docs/operations.md）。旧 ext4 副本 `~/ResearchOS` 已不再作为运行副本，仅作备份。
