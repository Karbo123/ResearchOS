...
- [~] `P0-PROJECT-UX-082` 将项目专属数据按项目目录隔离，并补齐项目删除、侧栏宽度和标签交互。
  ...
  - [x] `082b` 左侧项目列表悬停超过 3 秒显示三点图标，打开 Apple 风格删除确认对话框；必须输入完整项目名称和 `DELETE`，前后端双重校验后删除数据库记录、语义记忆、项目目录、关联产物和项目级配置；新增 API 与隔离测试。
...
- [~] `P0-PROJECT-UX-084` 优化项目删除入口和确认对话框的可见性、误操作防护与层级。
  - [x] `084a` 删除入口改为悬停后缓慢出现的红色垃圾桶图标，不占用未显示时的项目列表宽度；确认框禁止复制/粘贴项目名称，并统一确认词字体；已通过类型检查、UI 检查、构建和差异校验。
