...
- [~] `P0-IMPACT-008` 实现实体级影响分析、精确失效传播和检查点局部重跑。
  ...
  - 当前缺口：尚未自动创建主题专属重跑，也未在 Runner 中执行主题专属检查点恢复；因此任务继续保持 `[~]`。任何模型或主题 Runner 失败都直接返回结构化错误，不使用 fallback 或无关演示实验。
...
- 2026-07-30：完成本轮 `P0-IMPACT-008` 自动检查点提交：批准 `experiment_rerun` 后自动复用 `/api/experiments` 受控链，失败保留结构化错误并写入审计，前端移除二次执行按钮；主题专属 Runner 仍未实现，不能把它标记为已完成。API `56 passed`；文档同步、Schema/Compose/JSON、Idea case、API 容器编译、前端语法/UX、浏览器面板和 `git diff --check` 均通过。提交：`d1a91eb`。
