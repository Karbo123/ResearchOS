...
用户 2026-08-02 新增需求：在页面右上角提供浅色、暗色、浅彩色、深彩色四种主题的切换，使用 dropdown 选择；默认浅色主题；主题配色遵循苹果公司（Apple）的颜色设计规范。主题只改变视觉呈现，不改变布局、功能和数据。

- [~] `P0-UI-THEME-080` 右上角四主题切换与 Apple 规范配色。
  - [x] `080a` 在 `Topbar` 的 `.top-actions` 区域、语言切换旁边增加主题下拉，提供 `浅色`、`暗色`、`浅彩色`、`深彩色` 四档，默认 `浅色`；切换立即生效，不刷新页面，不影响当前 project/area/tab 和语言。
  - [x] `080b` 主题选择持久化到 `localStorage`（如 `researchos.theme`），刷新、前进/后退、重新打开页面保持；不写进 URL；默认浅色。
  - [x] `080c` 实现方式：把现有 `apple-glass.css` 中写死的颜色、阴影、毛玻璃参数收敛为语义 CSS 变量，通过根节点 `data-theme="light|dark|colorful"`（或等价机制）切换三套主题；组件不再内联写死颜色，统一使用语义变量。
  ...
  - [x] `080f` 彩色主题：在保持 Apple 克制与可读性的前提下，用更鲜明的主色/强调色区分研究状态、图表、标签、进度和不同工作包；不能乱用高饱和色导致看不清或降低专业感。
  - [x] `080g` 覆盖全部界面：卡片、表格、表单、Modal、Toast、tooltip、Markdown preview、图表/点云预览、diff、滚动条、进度条、选中态、hover、focus ring 在四种主题下均正确，文字与背景对比度至少满足 WCAG AA。
  - [~] `080h` 主题与语言、导航互不干扰：切换主题不改变语言，不改变 project/area/tab，刷新后恢复所选主题且无闪烁、无布局跳动。
  - [~] `080i` 测试与验收：四主题桌面/窄屏截图、颜色变量引用检查（禁止组件内联写死颜色）、持久化测试、切换无闪烁；截图验收并入 `070-A4`/`078l`。
  - [x] `080j` 完成后同步 `README.md`、`README.zh-CN.md`、`AGENTS.md`、架构/UI 文档和 `DOCS_SYNC_VERSION`，说明四主题、默认浅色和 Apple 配色规范。
  - [x] `080k` 已审计四套主题的正文、次要文字、成功/失败/警告状态、表单错误、图表标签和移动端 Modal 操作栏；残余硬编码文字色已迁移到语义变量，时间序列图随主题切换。代表性文字对比度均达到 WCAG AA：浅色次要文字 5.07:1、暗色次要文字 5.42:1、暗色错误文字 4.52:1。
  - [x] `080l` 已将浅色主题一级标签选中态改为蓝色玻璃层，不再使用纯黑背景；`colorful` 已拆分为 `colorful-light` 与 `colorful-dark`，分别使用协调的多色浅色/深色调色板，并兼容旧 `colorful` 存储值。
  - [x] `080m` 用户进一步明确彩色主题应使用红、黄、蓝、绿、紫、橙等离散卡片配色，而不是大面积彩色渐变；已为浅彩色和深彩色的指标卡、概览卡、产物卡、数据行、种子文献行和记忆结果建立六色循环表面与边框，移除彩色主题页面级渐变，并统一使用主题正文/次要文字变量。六色卡片上的正文对比度为 9.90:1–15.17:1，次要文字对比度为 5.15:1–6.10:1；`typecheck`、`ui:check`、Web `build`、`docs:check` 和 `git diff --check` 已通过。
