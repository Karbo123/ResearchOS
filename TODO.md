...
- [~] `P0-UI-THEME-080` 右上角三主题切换与 Apple 规范配色。
  ...
  - [x] `080g` 覆盖全部界面：卡片、表格、表单、Modal、Toast、tooltip、Markdown preview、图表/点云预览、diff、滚动条、进度条、选中态、hover、focus ring 在三种主题下均正确，文字与背景对比度至少满足 WCAG AA。
  ...
  - [x] `080j` 完成后同步 `README.md`、`README.zh-CN.md`、`AGENTS.md`、架构/UI 文档和 `DOCS_SYNC_VERSION`，说明三主题、默认浅色和 Apple 配色规范。
  - [x] `080k` 已审计三套主题的正文、次要文字、成功/失败/警告状态、表单错误、图表标签和移动端 Modal 操作栏；残余硬编码文字色已迁移到语义变量，时间序列图随主题切换。代表性文字对比度均达到 WCAG AA：浅色次要文字 5.07:1、暗色次要文字 5.42:1、暗色错误文字 4.52:1。
