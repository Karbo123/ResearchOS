...
MLflow plus MinIO remains the single experiment-tracking path for this Compose MVP. It already binds parameters, metrics, fixed resource samples, Run identity, artifacts, Git/data/seed identity and PostgreSQL lineage. W&B would add account/outbound-service and credential controls; TensorBoard would add a visualization stream without replacing the existing Run and artifact contracts. Neither is enabled by default. A future deployment that needs either must introduce an explicit, approval-gated architecture change and preserve MLflow/PostgreSQL as the state and provenance source.
