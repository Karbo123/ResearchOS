...
Project workflow source is trusted only after the normal approval chain. The workflow edit Agent emits a diff, the API applies it in a temporary workspace, previews it through the Mastra loader, and only an approved `code_patch` writes `workflow.ts` and commits it in the project Git. The loader rejects filesystem, process, network, dynamic import, eval, URL, and credential patterns and allows only `@research-os/workflow-kit`, `zod`, and Mastra workflow imports. Approved workflow code still runs inside the Mastra process, so it is treated as trusted local project code; a dedicated sandbox remains future work and must not be presented as VM isolation.
