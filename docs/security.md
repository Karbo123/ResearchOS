...
- Runner 只加入 Compose 的 `internal` `runner-internal` 网络；它不能通过默认网络访问其他服务，也没有外部网络出口。`internal-mlflow-only` 是当前任务模板的受限策略标签，API/MLflow 仍共享该内部控制网络；per-run 容器已由 launcher 创建，GPU 运行时、通用 Python/C++/Conda 环境和真正 volume 级磁盘配额仍是未完成能力。
