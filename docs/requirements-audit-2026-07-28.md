...
实验跟踪评估：当前单机离线部署继续使用 MLflow + MinIO 作为唯一跟踪和产物事实源。W&B 会增加账号/出站控制，TensorBoard 不能替代 PostgreSQL、Run、Artifact 和审批谱系；本轮不启用第二状态源，未来采用必须先经过独立架构评审。
