...
- 未授权恶意软件 Idea：`feasibility=blocked`，确认创建项目返回 409。
...
| 初始聊天、多轮澄清、ProjectSpec | 已实现（自适应 MVP） | 每轮 AI 整体更新草稿、推断明显领域并公开假设；默认全自动模式只问阻塞性关键信息，详细模式按相关缺口扩大了解范围，均不使用固定问卷；Luna/Terra/Sol 分级路由、等待进度、严格 Pydantic/JSON Schema 和显式安全降级已验证；信息不足不创建项目。测试输入只来自公开 `tests/idea-cases`。 |
...
| 可行性、重复、伦理、合规、资源风险 | 部分实现 | 危险/未授权目标可阻断；资源字段会澄清；重复研究与创新性只能做 DOI 元数据级初筛。 |
