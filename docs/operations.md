...
Project workflows live in `projects/<id>/workflow.ts` and are compiled by the Mastra process into `runtime/workflow-cache/<id>/workflow-<sha256>.mjs`. The loader polls every `RESEARCH_WORKFLOW_POLL_INTERVAL_MS` (default 500) milliseconds and uses `workflow.ts` plus workflow-kit source hashes to detect changes. Because `/mnt/d` has no inotify support, editing `workflow.ts` alone is hot reloaded, but changing loader or runtime code still requires restarting Mastra. Invalid or unsafe sources keep the previous active version and surface a structured error. Run records are atomically written to `runtime/workflow-runs.json`; restarting Mastra restores run-to-version mapping from that file and rebuilds versions from the compile cache. Deleting a project calls the Mastra workflow cleanup endpoint before removing the project directory.

The API report scheduler dispatches `{ action: 'reports' }` to each active project's workflow at the configured local times (`RESEARCH_REPORT_DAILY_TIME`, `RESEARCH_REPORT_WEEKLY_TIME`, `RESEARCH_REPORT_WEEKDAY`, `RESEARCH_REPORT_POLL_SECONDS`, `RESEARCH_REPORT_TIMEOUT_SECONDS`). Its state file `runtime/report-scheduler-state.json` prevents duplicate dispatch in the same daily/weekly window after a restart.
