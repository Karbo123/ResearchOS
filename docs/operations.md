...
The application database is intentionally pinned to `@electric-sql/pglite@0.3.10`. Version 0.3.7 is affected by the upstream `MessageContext` leak; 0.5.x failed to open a copied 0.3.x runtime directory during the 2026-08-06 upgrade check. Never test a PGlite upgrade against the live directory. Stop the API, copy the selected `RESEARCH_RUNTIME_DIR/research-os.pglite`, and verify open, representative reads, a transaction, and close with the candidate version before changing the lockfile.

Workflow and chat event streams use the local abort-aware `ReadableStream` implementation, not Hono's `streamSSE` helper or a patched dependency. A workflow stream must remain open after its initial snapshot and must stop polling immediately after cancellation. The UI deliberately has no REST polling fallback for workflow live updates, so a stream regression remains visible. The task worker uses one dispatcher with exponential idle backoff from 250ms to 5s and only heartbeats active worker IDs. Fatal PGlite/WASM failures terminate the API; repeated `workflow task worker recovery failed` messages indicate an invalid process and must not be treated as successful recovery.
...
Use the lower-left settings button or edit project `.env`. The panel has three top-level tabs: `General` (appearance only), `Models` (project-scoped), and `System` (proxy address information). Luna, Terra, and Sol are independent. A blank key in the Web form preserves the existing key. The settings API never returns key material. Project-scoped overrides (code tiers, document text, image recognition, image generation, Embedding, voice recognition) are stored in `projects/<project-id>/.researchos/model-settings.json` (mode 0600, atomic write, removed with the project, ignored by the project Git workspace); without an override the project falls back to `.env` defaults. The proxy address remains in `runtime/model-settings.json`; each model's `use_proxy` flag is project-scoped.
...
The `System` tab stores only the proxy address and provides a proxy connectivity test against `https://example.com`. The address itself is persisted in `runtime/model-settings.json`; it is no longer a global on/off switch. Each model page has its own project-scoped proxy switch: code tiers, document text, and Embedding default to direct, while image recognition, image generation, and voice recognition default to using the proxy. Mastra model calls, model catalog requests, voice transcription, and remote Embedding egress honor the relevant model switch. Loopback and RFC1918 model targets always stay direct so the local gateway and bridge are never proxied. The default proxy address is derived from `HTTPS_PROXY`/`HTTP_PROXY` (or their lowercase forms) at startup for compatibility.

## Workflow Operations

Project workflows live in `projects/<id>/workflow.ts` and are compiled by the native API loader into `runtime/workflow-v2-cache/<id>/workflow-<sha256>.mjs`. The loader polls every `RESEARCH_WORKFLOW_POLL_INTERVAL_MS` (default 2000) milliseconds and uses the source SHA-256 to detect changes. Because `/mnt/d` has no inotify support, editing `workflow.ts` alone is hot reloaded, but changing loader or runtime code still requires restarting the API. Invalid or unsafe sources keep the previous active definition version and surface a structured error. Definition, runtime, event, and node-run state lives in PGlite (`workflow_definitions`, `project_workflow_runtime`, `workflow_events`, `workflow_node_runs`, and task rows); startup recovery re-leases stale coordinator locks, re-queues interrupted tasks, and dispatches unprocessed events. Deleting a project removes these rows plus the `runtime/workflow-v2-cache/<id>/` directory before removing the project directory.

The API report scheduler appends `report.window.reached` events for active projects at the configured local times (`RESEARCH_REPORT_DAILY_TIME`, `RESEARCH_REPORT_WEEKLY_TIME`, `RESEARCH_REPORT_WEEKDAY`, `RESEARCH_REPORT_POLL_SECONDS`, `RESEARCH_REPORT_TIMEOUT_SECONDS`). Its state file `runtime/report-scheduler-state.json` prevents duplicate events in the same daily/weekly window after a restart.

## Voice Recognition

The chat voice input can use either the browser's Web Speech API or a Groq Whisper transcription endpoint (`RESEARCH_VOICE_PROVIDER=browser|groq`). `GROQ_API_KEY` from the project `.env` is the default key; the web UI can also override the Groq model, API URL, key, and proxy switch per project. Runtime-provided keys are stored in `projects/<project-id>/.researchos/model-settings.json` (mode 0600) and never returned by the settings API, which only reports `key_configured`. Leaving the key blank in the UI keeps the existing key. When the browser engine is selected the UI does not ask for a Groq model, API URL, key, or proxy switch; those fields appear only for the Groq engine. Groq transcription follows the voice model's own proxy switch, which is required on networks where `api.groq.com` is not directly reachable.
