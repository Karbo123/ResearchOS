...
### Per-project embedding isolation

Embedding configuration is per-project. A project without an override uses the global default instance (`SUPERMEMORY_BASE_URL`, normally `127.0.0.1:6767`) with the global `.env` embedding variables. A project with an explicit override (`GET/PUT /api/projects/<project-id>/embedding-settings`) gets a **dedicated Supermemory instance** on its own port (allocated from 6770–6869) with its own encrypted data directory under `runtime/supermemory/projects/<project-id>/data`, so different projects can use different providers/models/dimensions without vector-space contamination. Overrides are stored in `runtime/project-embedding-settings.json` (mode 0600, atomic write, same pattern as `model-settings.json`); the API never returns the key, only `key_configured`. The instance is started lazily on first memory request and health-checked before use; the binary must be `server-v0.0.5` when a remote provider is configured, otherwise startup fails closed. Switching provider/model/dimensions requires a fresh data directory: the API refuses with `embedding_requires_reset` (409) unless the caller confirms `reset_data: true`, in which case the old data directory is renamed to `<data>.bak-<timestamp>` (recoverable) and the project's semantic memory must be re-ingested. Benchmark on 2026-08-01 (same query, same corpus, isolated instances): local `Xenova/bge-m3` 1024d embeds a short text in 30–72ms with a full search at 58–159ms; remote `Qwen3-Embedding-8B` (gitee) embeds in ~120–210ms with a full search at 286–653ms; the global default remains local bge-m3. End-to-end recheck through the public API: local full search 86–172ms vs remote full search 662–1151ms (local ~8–10× faster), with a local-override instance (6770) and a remote-override instance (6771) verified running side by side in WSL2.
