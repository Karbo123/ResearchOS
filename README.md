...
| General research autonomy | **Partial / roadmap** | General Python/C++/Conda/GPU jobs, independent per-run containers, automatic checkpoint reruns, external notifications, evidence-grounded Related Work, and full paper writing remain open. The current Runner has allowlisted non-root child-process isolation with CPU, memory, PID, timeout, and per-run disk limits. Official GitHub/GitLab repository verification and approval-gated fixed-commit import are implemented. |
