...
8. In the project overview, use **Generate evidence-grounded paper draft** only after verified page-level evidence has been ingested. The API creates a reviewable `paper/main.tex` replace patch Proposal bound to the current Idea version, evidence IDs, and recorded successful runs. It rejects metadata-only evidence, leaves absent results explicitly unexecuted, and requires approval before any file write or LaTeX compile.
