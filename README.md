...
4. Confirm the specification. Research OS creates a UUID, Git workspace, project directories, Idea v1, database state, checkpoints, and one idempotent durable PostgreSQL task for the n8n main workflow; `queue-worker` leases and dispatches it.
...
| `QUEUE_POLL_SECONDS`, `QUEUE_LEASE_SECONDS`, `QUEUE_WORKFLOW_TIMEOUT_SECONDS`, `QUEUE_RETRY_BASE_SECONDS`, `QUEUE_RETRY_MAX_SECONDS` | Compose defaults | Durable queue polling, lease duration, fixed n8n request timeout, and bounded exponential retry delays. Queue kinds and payloads remain API allowlisted; duplicate idempotency keys reuse the existing task. |
...
The highest-value unfinished work is tracked in [`TODO.md`](TODO.md): real GPU-host validation, richer material libraries, interactive 3D viewing, complete evidence-grounded LaTeX writing, and signed clean-VM validation of the single-EXE installer. The durable project bootstrap/resume queue is implemented with idempotency, leases, bounded retries, stale-lease protection, and audit records; persistent queueing for the separate Runner execution chain remains outside this task. Controlled topic-specific Python, fixed micromamba/Conda Python, C++/CMake, and an allowlisted GPU request now use per-run containers with hard output-volume limits. Approved checkpoint rerun Proposals auto-submit through the matching fixed template and remain structured failures when submission fails. Approved project changes now produce a reviewable dependency graph and pending rerun Proposals for safe terminal checkpoints; those Proposals still require explicit approval. Deterministic daily/weekly reports include operational metrics and can send one explicitly requested HTTPS webhook when enabled. Official repository/license verification and controlled fixed-commit download are implemented with approval gating. RAGFlow/LlamaIndex and LangGraph are deliberately deferred until scale or workflow complexity justifies them.
