...
本文档是替换工作的实施合同。它描述计划，不代表 Rust 服务已经接入、数据已经迁移或官方二进制已经退役。任务状态、范围、阻塞和验证结果必须在开发过程中即时写回本文档；只有代码、测试、运维、文档和真实验证全部完成，才能把总任务标为 `[x]`。
...
- [x] `SMRS-DECISION-004` 通过 `git subtree` 把源码引入仓库内的 `services/supermemory-rs/`，不使用 submodule，不保留嵌套 `.git`。
- [x] `SMRS-DECISION-005` 现有 Research OS TypeScript 调用是兼容契约：先保证当前 `supermemory-service.ts`、Mastra processor、端口、环境变量和项目级 Embedding 配置池的调用方式不变，再修改 Rust 直到现有调用不需要改动。
- [x] `SMRS-DECISION-006` 不迁移旧向量；模型、维度或实现变化时从权威源重建派生索引。
- [x] `SMRS-DECISION-007` 不引入 outbox、通用 `semantic_index_bindings` 表、`/researchos/v1` 私有 API、官方/shadow/rust 三态模式、服务 token 架构或强制观察期。只有真实兼容工作证明某一样必不可少时，才作为独立任务重新评估。

## 2. 为什么要替换

当前官方二进制造成的限制不是普通配置问题：

1. 服务端源码不在官方公开仓库中，关键行为只能通过黑盒、打包产物和人工字节补丁推断。
...
5. 向量维度、超时、处理流程和错误语义受闭源实现约束，难以审计，也无法为 Memory v2 做可靠演进。

目标不是重新造一个无限扩张的记忆平台，而是获得一套源码可读、行为可测、配置可控、可按项目隔离且能被 Research OS 完整审计的本地语义索引服务。
...
| memory 列表 | `POST /v4/memories/list` | 缺失 | 实现 Research OS 实际使用的分页与 document ID 过滤 |
...
| Mastra 对话写入 | `POST /v4/conversations` | 缺失 | 保留现有调用路径；如当前代码确实调用，则实现最小兼容端点，不引入 outbox |
| PDF/图片摄取 | `documents.uploadFile` | 缺失 | 移至 Research OS 受控提取层，Rust 只接收提取后的文本和 provenance |
| 项目级 Embedding | 6767 + 6770–6869 配置池 | 缺失 | 新增 local/OpenAI-compatible backend 并复用现有配置池运行合同 |
...
现有 `AGENTS.md` 的“业务应用、数据库迁移、运维脚本和测试只使用 TypeScript”与本决策存在冲突。实施前做一次窄范围修订：
...
- 所有依赖固定在 `Cargo.lock`；CI/本地执行 `cargo fmt`、`cargo clippy -D warnings`、`cargo test --locked`、`cargo audit` 和 `cargo deny`。
...
| backend document ID | PGlite memory link + Rust SQLite | 两边保存映射，PGlite决定当前有效版本 |

## 7. Rust 二次开发范围

### 7.1 Embedding

新增统一异步 `EmbeddingBackend`，至少实现：
...
这样可以移除官方二进制中的 Mistral/Gemini 固定依赖，并使图片模型、代理和 key 真正遵循项目设置。

### 7.4 对话记忆

- PGlite messages 保持对话权威源。
- 保留现有 Mastra output processor 对语义记忆的写入路径；如果当前代码调用 `/v4/conversations`，在 Rust 中实现该端点所需的最小契约，不引入第二套重复写链路或 TypeScript outbox。
- 工作区 `area/tab` 继续写入 metadata，并在 Rust 搜索阶段作为严格 filter。
- 对话删除/项目删除必须同步产生撤销任务；不能只删 PGlite 行而留下可搜索文本。

### 7.5 memory extraction 和 Responses
...
- 在现有 `DELETE /v4/memories` 之外，按 source document ID 提供确定性 revoke：使源文档、chunks 和 embeddings 立即退出所有搜索，并处理仅由该文档支持的 memory entity；不依赖重新调用 LLM。
- 单文档删除、容器范围清理和 acceptance 清理在现有兼容端点中实现，不新增独立私有 API。
...
配置合同沿用现有环境变量；新增字段必须同步 `.env.example` 和严格 Zod schema：
...
- 数据接口沿用现有 loopback 自动认证/API key 机制，不新增运行时 token 架构；只有 `/health`、`/ready` 和静态 API reference 可以匿名读取。
- 每个数据请求携带经过校验的项目 slug；Rust 必须验证请求项目、container tag `research-os-project-<slug>` 和 metadata `project_id` 三者完全一致。
- container tag 不允许客户端任意拼接多个项目；管理型 bulk 操作使用现有后端认证 key，不新增 operator token。
...
3. 每个项目生成迁移报告：计划、成功、失败、跳过、缺源、hash 和隔离探针。
4. 对现有文档容量语料、中文/英文/混合文本和至少两个项目完成真实搜索验收。
5. 暂停新的索引写入，等待 worker 清空，执行最后增量重建。
6. 显式把启动入口切为 Rust，将 Rust global 实例绑定 6767，重启 API/Mastra 后运行完整验收。
7. 官方实例保持停止但数据只读保留，不允许两个服务同时占用同一端口或数据目录。

不默认引入 shadow 双写或观察期；如真实兼容验收证明需要，再以独立任务评估。

### 9.3 回滚

- 回滚只能由 operator 显式执行，不能在请求失败时自动发生。
- 切换前保留官方数据快照、配置快照、PGlite 备份和切换 manifest。
- 如果出现跨项目泄漏、不可恢复数据损坏、持续搜索错误或严重性能回退，停止写入、保存 Rust 数据与日志、恢复明确的官方快照并从切换点之后的权威源重新摄取。
...
- [ ] `SMRS-P0-004` 把本计划涉及的新环境变量、目录、端口、备份和失败语义加入文档草案，但尚未实现的内容必须标记 planned。
...
- [ ] `SMRS-P2-002` 复用现有 loopback 自动认证/API key，实现项目 slug/container/metadata 三重校验和管理操作隔离。
- [ ] `SMRS-P2-003` 补齐确定性 request ID、结构化错误、status/revision/job 状态机和幂等测试。
- [ ] `SMRS-P2-004` 对当前 npm `supermemory` SDK 运行契约测试；只保证已登记调用，不伪造全 API 兼容。
...
- [ ] `SMRS-P4-003` 保留当前对话语义记忆路径；如现有代码调用 `/v4/conversations`，实现最小兼容端点，验证重放/幂等/撤销。
...
退出门槛：PDF/图片不再触发固定 provider；对话不引入重复写链路；Graph 无虚构边；撤销不依赖 LLM 是否抽取 entity。

### Phase 5：运维、切换和退役

- [ ] `SMRS-P5-001` 改造 `supermemory-service.ts`、Mastra processor 和 indexing service，使所有请求走统一严格客户端适配层；现有调用语义保持不变。
- [ ] `SMRS-P5-002` 改造 start/stop/status/monitor/backup/recovery 脚本，支持 global 与共享配置池 Rust 实例。
- [ ] `SMRS-P5-003` 实现 SQLite WAL 一致性备份、`PRAGMA integrity_check`、manifest SHA-256 和单独目录恢复演练。
- [ ] `SMRS-P5-004` 从权威源重建 active 索引，在独立端口/目录运行并完成 TypeScript 契约、项目隔离和真实模型验收。
- [ ] `SMRS-P5-005` 维护窗口内暂停写入、完成最后增量重建，显式切换到 Rust 并运行全量 Research OS 验收。
- [ ] `SMRS-P5-006` 停止并归档官方实例；删除 `SUPERMEMORY_SERVER_BIN`、v0.0.5 build 检查、字节补丁逻辑、旧 bridge 兼容分支和过时错误文案。
- [ ] `SMRS-P5-007` 同步 `AGENTS.md`、README 双语、`.env.example`、architecture、operations、security、Memory v2 和主 TODO，运行完整验证并创建 Conventional Commit。
...
- 普通项目请求不能 list/purge 其他 container；浏览器不直接获得后端认证 key。
...
1. Research OS 当前 TypeScript 调用在 Rust 服务上无需语义改动即可工作。
2. 本地 bge-m3 和远程 OpenAI-compatible Embedding 均真实通过，配置不同的项目不混用向量空间。
3. 项目、workspace、generation 和来源验证均失败关闭，跨项目泄漏测试为零。
4. 模型/Embedding/提取失败不产生 fallback 内容、伪 memory、空成功或旧结果冒充。
5. 确定性撤销、容器清理、项目删除和重建均走通真实链路，PDF/图片不再触发固定 provider。
6. 官方闭源二进制、人工字节补丁和固定 build 检查不再参与正常启动、配置池或验收。
7. README 双语、AGENTS、Memory v2、架构、运维、安全、环境模板和主 TODO 与真实实现一致。
8. Git 只提交源码、migration、测试和文档；key、数据库、模型权重、运行日志和备份不进入版本库。
