...
Embedding 配置是**项目级且完全隔离**的。没有覆盖的项目使用全局 `.env` 默认值（共享实例 `127.0.0.1:6767`）。项目级覆盖（网页左下角设置 → Embedding）会为该项目启动一个**独立 Supermemory 实例**（独立端口 6770–6869 + 独立加密数据目录 `runtime/supermemory/projects/<project-id>/data`），不同项目可以使用不同的 provider、模型和维度，向量空间互不污染。覆盖配置保存在 `runtime/project-embedding-settings.json`（0600 权限、原子写入）；API 永不返回 key，只返回 `key_configured`。切换模型或维度必须使用全新数据目录：API 会以 `embedding_requires_reset`（409）拒绝，直到确认 `reset_data: true`，随后旧数据目录保留为 `.bak-<时间戳>` 备份，该项目的语义记忆需要重新摄入。2026-08-01 基准（同 query 同语料、隔离实例）：本地 `Xenova/bge-m3` 1024 维单条嵌入 30–72ms、完整检索 58–159ms；远程 `Qwen3-Embedding-8B`（gitee）单条嵌入约 120–210ms、完整检索 286–653ms——**全局默认保持本地 bge-m3**。同日经公开 API 的端到端复核：本地完整检索 86–172ms（纯嵌入 54–68ms）、远程完整检索 662–1151ms，本地快约 8–10 倍；本地覆盖项目（端口 6770）与远程覆盖项目（端口 6771）各自独立实例并行运行、互不干扰。
