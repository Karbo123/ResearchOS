...
页面右上角提供语言与主题控件。语言支持简体中文（默认）、繁体中文、英语和西班牙语，选择保存在本地并在刷新后保持；界面文案全部走 i18n key 与统一术语表，聊天回复、报告/论文正文和实验日志等模型或数据动态内容保持原文，不自动翻译。主题提供浅色（默认）、暗色和彩色三档，使用 Apple System Colors 与语义 CSS 变量实现毛玻璃、分层阴影和弹簧动效；三档主题与四种语言均可通过真实浏览器加载，最终截图级视觉验收仍在 `P0-UI-THEME-080` 与 `P0-UI-I18N-079` 中跟踪。
