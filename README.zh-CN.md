...
项目工作区使用可读的无井号地址，例如 `/project/cnn-minimal-2q95/overview/overview`。内部 UUID 仍是项目不可变的身份标识，用于 API 权限和数据隔离，但正常浏览器地址不再显示它。创建项目时，缩写使用两个语义英文小写单词加四位小写字母/数字随机后缀，服务器检查完整标识是否唯一。用户手动填写时也必须遵守 `word-word-xxxx` 格式。历史三词地址、UUID 地址和旧 hash 地址仍可兼容访问，不会把数据库中的历史 slug 强行改名。
