...
describe('project deletion isolation', () => {
  beforeAll(async () => {
    ...
    writeFileSync(`${projectRoot(projectId)}/workflow.ts`, `const workflow = ${JSON.stringify({
      schemaVersion: 2,
      templateVersion: 'delete-test@1',
      groups: [{ id: 'test', label_key: 'test' }],
      nodes: [{ id: 'a', group: 'test', capability: 'noop', label_key: 'a', retry: { max_attempts: 1, backoff_seconds: 1 }, timeout_seconds: 10, concurrency: 'parallel' }],
      edges: [],
      triggers: [{ event_type: 'test.start', node_id: 'a', mode: 'root' }],
  ...
  it('removes pinned project records, files, and the project directory', async () => {
    ...
    for (const table of ['workflow_events', 'workflow_node_runs', 'workflow_definitions', 'project_workflow_runtime']) {
      await expect(database.query(`SELECT project_id FROM ${table} WHERE project_id=$1`, [projectId])).resolves.toMatchObject({ rows: [] })
    }
    expect(existsSync(resolve(runtimeRoot, 'workflow-v2-cache', projectId))).toBe(false)
  })
})
