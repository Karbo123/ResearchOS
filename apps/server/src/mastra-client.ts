...
export async function mastraGet<T>(path: string): Promise<T> {
  let response: Response
  try {
    response = await fetch(`${mastraBase()}${path}`, {
      method: 'GET',
    ...
    const code = error instanceof DOMException && error.name === 'TimeoutError' ? 'llm_timeout' : 'llm_request_failed'
    throw new ApiError(502, code, code === 'llm_timeout' ? '模型请求超时。' : 'Mastra 请求失败。')
  }
  const raw = await response.text()
  let parsed: unknown
  try {
  ...
  const result = parsed as Record<string, unknown>
  if (!response.ok) throw new ApiError(response.status === 503 ? 503 : 502, String(result.code || 'llm_request_failed'), String(result.message || 'Mastra 请求失败。'))
  return result as T
}

export async function mastraDelete<T>(path: string): Promise<T> {
  let response: Response
  try {
    response = await fetch(`${mastraBase()}${path}`, {
      method: 'DELETE',
    ...
    const code = error instanceof DOMException && error.name === 'TimeoutError' ? 'llm_timeout' : 'llm_request_failed'
    throw new ApiError(502, code, code === 'llm_timeout' ? 'Mastra 请求超时。' : 'Mastra 请求失败。')
  }
  const raw = await response.text()
  let parsed: unknown
  try {
