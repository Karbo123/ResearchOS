...
import { indexKnowledgeDocument, indexUploadedMaterial } from './indexing-service.js'
...
async function runTask(task: Task): Promise<void> {
  ...
  if (task.kind === 'material_index') {
    ...
    await indexUploadedMaterial(task.project_id, uploadedFileId, task.id)
    return
  }
  if (task.kind === 'knowledge_reindex') {
    const documentId = typeof task.payload.document_id === 'string' ? task.payload.document_id : ''
    if (!documentId) throw new Error('knowledge_document_id_missing')
    await indexKnowledgeDocument(task.project_id, documentId)
    return
  }
...
export function isTaskErrorTerminal(error: unknown, attempts: number, maxAttempts: number): boolean {
  return error instanceof ApiError || attempts >= maxAttempts
}
...
async function executeClaimedTask(task: Task): Promise<void> {
  try {
    ...
    const terminal = isTaskErrorTerminal(error, task.attempts, task.max_attempts)
    if (task.kind === 'workflow_node_task') {
