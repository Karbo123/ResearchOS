    ...
    <>
      ...
      <div className="section comparison-form-panel">
        ...
        <div className="form-grid two-up">
          ...
          <label>{t('comparison.reproductionRun')}<select value={runId} onChange={event => setRunId(event.target.value)}><option value="">{t('common.select')}</option>{runs.map(run => <option value={run.id} key={run.id}>{t('overview.runDetail', { run: run.id.slice(0, 8) })} · {t('reproduction.seeds')} {run.random_seeds.join(', ')}</option>)}</select></label>
        </div>
...
function ComparisonCard({ comparison, busy, onDecide }: { comparison: ResearchComparison; busy: boolean; onDecide: (comparisonId: string, candidateId: string, decision: 'accepted' | 'rejected') => Promise<void> }) {
  ...
  return (
    <article className="data-row comparison-card">
      <div className="comparison-card-heading"><div><h3>{t('comparison.paper')} {comparison.paper_id.slice(0, 8)} · {t('overview.runDetail', { run: comparison.reproduction_run_id.slice(0, 8) })}</h3><p>{t('comparison.inputHash')} <code>{comparison.input_hash || t('common.unrecorded')}</code></p></div><Badge status={comparison.status}>{t(statusLabel(comparison.status) as TranslationKey)}</Badge></div>
      ...
      <div className="comparison-provenance"><span>{t('literature.commit')} <code>{String(reproduction.source_commit || t('common.unrecorded'))}</code></span><span>{t('reproduction.seeds')} {Array.isArray(reproduction.random_seeds) ? reproduction.random_seeds.join(', ') : t('common.unrecorded')}</span><span>{t('comparison.artifactsCount', { count: artifacts.length })}</span></div>
      {metrics.length ? <div className="comparison-metrics"><table><thead><tr><th>{t('comparison.metric')}</th><th>{t('comparison.paper')}</th><th>{t('comparison.reproductionMean')}</th><th>{t('comparison.std')}</th><th>{t('comparison.delta')}</th><th>{t('comparison.status')}</th></tr></thead><tbody>{metrics.map(([name, metric]) => <tr key={name}><td>{name}</td><td>{t(metricNumber(metric.paper_value) as TranslationKey)}</td><td>{t(metricNumber(metric.reproduction_mean) as TranslationKey)}</td><td>{t(metricNumber(metric.reproduction_population_std) as TranslationKey)}</td><td>{t(metricNumber(metric.delta) as TranslationKey)}</td><td><Badge status={metric.status}>{t(statusLabel(metric.status) as TranslationKey)}</Badge></td></tr>)}</tbody></table></div> : null}
